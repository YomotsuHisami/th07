...
void EffectManager::ShiftEffectsAfterCameraTeleport(ZunVec3 *param_1)
{
    i32 i;
    Effect *effect;
...
Effect *EffectManager::SpawnSpecialEffect(i32 effectId, ZunVec3 *pos, i32 param_3, i32 param_4,
                                         u32 color)
{
    (void)param_4;
...
u32 EffectManager::OnDraw(EffectManager *arg)
{
    auto sortAndDraw = [](Effect *layerHead, bool isBillboard) {
        ...
        {
            active[i]->vm.pos = active[i]->prevPos.Lerp(active[i]->pos, g_RenderAlpha);
#ifdef TH_ENABLE_MULTIPLAYER_GAMEPLAY
...
i32 EffectManager::DrawLayer1Effects()
{
    ...
    while (effect)
    {
        ...
        effect->vm.pos = effect->prevPos.Lerp(effect->pos, g_RenderAlpha);
