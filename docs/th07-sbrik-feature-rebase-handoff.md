...
The combined `tools/netplay/lan-relay.cjs` remains the current development lobby/signaling/emergency-relay server. It can inject ICE configuration and can generate coturn TURN REST-style temporary credentials from `TH07_TURN_URLS` + `TH07_TURN_SHARED_SECRET` (+ optional `TH07_TURN_TTL_SECONDS`). The TURN shared secret never enters launcher source or runtime options. Default STUN remains `stun:stun.cloudflare.com:3478` when the server does not override it.
